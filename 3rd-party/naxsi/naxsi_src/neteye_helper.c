			   ...
			   void *conf)
{
    ...
    /* parse log */
    if (ngx_strstr(value[3].data, "log=") != NULL) {
        if ((value[3].len - strlen("log=")) == 0) {
            return "not valid log option";
        } else {
            if (ngx_strstr(value[3].data, "on")) {
                neteye_action->log = 1;
            } else {
                neteye_action->log = 0;
            }
    ...
    /* parse notify */
    if (ngx_strstr(value[4].data, "notify=") != NULL) {
#if (NGX_HTTP_STATUS_PAGE)
        if ((value[4].len - strlen("notify=")) == 0) { 
            neteye_action->error_page.len = 0;
            neteye_action->error_page.data = NULL;
        } else {
            neteye_action->error_page.data = value[4].data + strlen("notify=");
            neteye_action->error_page.len = value[4].len - strlen("notify=");
    ...
    /* parse logid */
    if (ngx_strstr(value[5].data, "logid=") != NULL) {
        if ((value[5].len - strlen("logid=")) == 0) {
            return "not valid log id";
        } else {
            neteye_action->logid = ngx_atoi(value[5].data + strlen("logid="), 
                    value[5].len - strlen("logid="));
            if (neteye_action->logid == NGX_ERROR) {
                return "not valid log id";
            }
        }
    } else {
        ...
        ngx_http_request_ctx_t *ctx, char *fmt)
{
    ...
    if (nlcf->neteye_actions->nelts == 0) {
